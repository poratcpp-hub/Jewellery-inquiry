        ...
        {orders.map(order => (
          <div key={order.id} className="flex items-center justify-between p-4 hover:bg-[#faf8f5] transition-colors">
            ...
            <div className="text-left mr-4 shrink-0">
              <p className="text-sm font-semibold text-[#2c1810]">{formatCurrency(order.sale_price)}</p>
              <p className="text-xs text-[#7a6a52]">{order.delivery_date ? formatDate(order.delivery_date) : '—'}</p>
            </div>
          </div>
        ))}
      </div>
